...
Based on Ted's izh.mod which was based in part on spikeout.mod from
the NEURON code for Brette et. al.
...
Also has the synaptic input described in the 2003 IEEE Transactions article
where synaptic inputs are modeled as an increase/decrease of I for a single
time step.

Also has the gaussian noise from Ted's ingauss mod file
...
INITIAL {
  ...
  net_send(0, 5) : activate Inoise
}

NET_RECEIVE(w) {
  if (flag == 0) { : synatpic input
    Isyn = w
:    printf("flag = %f \tw = %f \ttime = %f\n", flag, w, t)
  } else {
    Isyn = 0
  }
